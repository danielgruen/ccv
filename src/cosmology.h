...
namespace Ulinear {
 ...
 static double *logr=0;
 static double *logU=0;
 
 static double alphalarge; // power-law slope at large r
 
 const double logrstep=0.001;
 const double logrmin=-3;
 const int nsteps=5001;
 
 void initialize()
 {
  std::ifstream in("lut/U.tab");
  assert(in.is_open());
  ...
  alphalarge=(logU[nsteps-1]-logU[nsteps-6])/5.;
  
  assert(alphalarge>0);  
 }
 
 double Ulog(double log10r)
 // log10(r/comoving h^-1 Mpc)
 // output: h^{-3} Mpc^3
 {
   assert(log10r>=logrmin);
   
   if(!logr) initialize();
  
   double n=(log10r-logrmin)/logrstep;
   if(n>=0 && n<nsteps-1) { // linear interpolation in log r
     return pow10(logU[int(n)]*(1.-n+int(n))+logU[int(n)+1]*(n-int(n)));
   }
   else if(n>=nsteps-1) 
     return pow10(logU[nsteps-1]+alphalarge*(n-nsteps+1.)); // power law at large r
     
   assert(1==2); // never should happen
 }
...
namespace halomodel {
 ...
 {   
  ...
  double deltac_bryan_norman(double z)
  {
   double x = OmegaM*pow(1.+z,3)/(OmegaM*pow(1.+z,3)+OmegaL);
   return 18.*pow(M_PI,2)+x*(60.-32.*x);
  }
  
  double deltam_bryan_norman(double z)
  {
   return deltac_bryan_norman(z)*(1.+OmegaL/(OmegaM*pow(1.+z,3)));
  }
  
  double conz(double c1, double c2, double delta1Odelta2)
  {
    // if this is negative, your c2 needs to increase 
    // if this is positive, your c2 needs to decrease 
    // if this is zero, your c2 is just perfect
  return (log(1.+c1)-c1/(1.+c1))/(log(1.+c2)-c2/(1.+c2)) - pow(c1/c2,3)*delta1Odelta2;
  }

  double convert_concentration(double c_in, double delta_inOdelta_out, double precision=1e-6)
  // returns concentration w.r.t. overdensity delta_out
  // where c_in is the concentration at overdensity delta_in
  {
    if(fabs(log(delta_inOdelta_out))<0.001) return c_in; // deltas are less than 1 per mille different
    
    double c_above=c_in+0.1;
    double c_below=c_in-0.1;
    
    while(conz(c_in,c_above,delta_inOdelta_out)<0)
    {
      c_above+=0.1;
    }
      
    while(conz(c_in,c_below,delta_inOdelta_out)>0)
    {
      c_below-=0.1;
    }
    
    while(c_above-c_below>precision) 
    {
      double c_test=(c_above+c_below)/2.; 
      double test=conz(c_in,c_test,delta_inOdelta_out);
      if(test<0) c_below=c_test;
      else if(test>0) c_above=c_test;
    }
    
    return (c_above+c_below)/2.;
  }

  double convert_mass_deltam(double z, double M200m, double delta_out)
  // convert M=M_200 to M_delta_out, delta w.r.t. rho_m at redshift z
  {
    double rhom0 = 3.*H0*H0/(8.*M_PI*Gs) * OmegaM; // [ Msol / Mpc^3 ]
    double rhomz = rhom0 * pow(1.+z,3);
    double r200m = pow(3.*M200m/(200.*4.*rhomz*M_PI),1./3.);
    double c200m = halomodel::cmz_200m_duffy(M200m, z);
    double rs    = r200m/c200m;
    
    double c_out=convert_concentration(c200m, 200./delta_out);
    double r_out=c_out*rs;
    double m_out=delta_out*rhomz*4./3.*M_PI*pow(r_out,3);
    return m_out;
  }

  double convert_mass_deltam(double z, double M, double delta_in, double delta_out) 
  // convert M=M_delta_in to M_delta_out, delta w.r.t. rho_m at redshift z
  {
    double rhom0 = 3.*H0*H0/(8.*M_PI*Gs) * OmegaM; // [ Msol / Mpc^3 ]
    double rhomz = rhom0 * pow(1.+z,3);
    double r_in = pow(3.*M/(delta_in*4.*rhomz*M_PI),1./3.);
    
    // (1) find concentration c_in such that c_200m matches Duffy et al.
    //     do this by probing masses M200m until they fit
    
    double logm200mlow  = log10(M)-3.;
    double logm200mhigh = log10(M)+3.;
    double tolerance = log10(1.001);
    
    while(logm200mhigh-logm200mlow > tolerance)
    {
      double logm200mtest = (logm200mhigh+logm200mlow)/2.;
      double m200mtest = pow10(logm200mtest);
      double c200mtest = halomodel::cmz_200m_duffy(m200mtest, z);
      double M_test    = convert_mass_deltam(z,m200mtest,delta_in);
      if(M_test>M) logm200mhigh=logm200mtest;
      else logm200mlow=logm200mtest;
      
    }
    
    double m200m = pow10((logm200mhigh+logm200mlow)/2.);
    double c200m = halomodel::cmz_200m_duffy(m200m, z);
    double r200m = pow(3.*m200m/(200.*4.*rhomz*M_PI),1./3.);
    double rs    = r200m/c200m;
    
    // (2) convert to output delta_out (w.r.t. rho_m)
