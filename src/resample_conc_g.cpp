// compress an covariance matrix (conc) to the system of annuli used, transform to gamma
...
int main(int argc, char **argv)
{
  ...
  Matrix<double> covhatg(nv+1,nv+1); // covariance matrix extrapolated to include central covariance in last entry
  
  {
    std::ostringstream ss;
    ss << nc;
    cout << "opening templates/conc/cov_"+ss.str()+".fits" << endl;
    auto_ptr<FITS> pInfile(new FITS("templates/conc/cov_"+ss.str()+".fits",Read,true));
