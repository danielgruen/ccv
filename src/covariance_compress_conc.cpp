// compress an covariance matrix (conc) to the system of annuli used in the analysis for z=0.24533
...
int main(int argc, char **argv)
{
  ...
  if(argc!=3)
  {
   cout << "syntax: " << argv[0] << " [log M * 100] [destination]" << endl;
   return 0;
  }
...
#include "snippet/readhaloes_024533.cpp"
  ...
  double m200m=pow10(atof(argv[1])/100.);
  ...
  
  {
    ...
    ss << nc;
    cout << "opening ../conc/conc/cov_"+ss.str()+".fits" << endl;
    auto_ptr<FITS> pInfile(new FITS("../conc/conc/cov_"+ss.str()+".fits",Read,true));
