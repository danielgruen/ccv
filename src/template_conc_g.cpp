...
double Tmean[nv];	// mean of DeltaSigma[v]
double Tbuf[nv];	// buffer of DeltaSigma[v]
double Tprod[nv][nv]; // mean product of DeltaSigma[v_i][v_j]
...
int main(int argc, char **argv)
{
  ...
  {    
    ...
    for(int i=0; i<nv; i++) 
    {
      ...
      Tbuf[i] = rho0/c*nfw_gamma(ror200m*c)*w; // need to scale this later by r200m*(1+z)^3 and evaluate at radius in units of r200m
    }
    for(int i=0; i<nv; i++) 
    {
      Tmean[i] += Tbuf[i];    
